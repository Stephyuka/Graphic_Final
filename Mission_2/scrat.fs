...
uniform samplerCube texture_reflect;
...
float shadow_caculation(vec4 light_space)
{
    ...
    float closest_depth = texture(shadow_map, coords.xy).r; //get closest depth from light perspective
...
void main()
{
    ...
    shadow = min(shadow, 0.75); // keep the shadow down
    
     Color = vec4((ambient + (1.0 - shadow) * (diffuse + specular)) * color_r , 1.0f);
}
